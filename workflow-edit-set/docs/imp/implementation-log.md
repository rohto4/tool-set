...
- Session01 として初回ヘルプモーダルの表示制御と既読フラグを追加
- 重複していた `loadSampleButton` の識別子を解消し、テンプレート導線を安定化
- ヘルプモーダルのフォーカス復帰、スクロール抑止、ダイアログ属性を追加
- ファイル読込や複数選択など主要導線の残存英語メッセージを日本語化
- ヘルプカードのレスポンシブ崩れを抑える CSS を追加
- 進捗レビューを `site/review.html` へ分離し、メイン画面からは撤去
- パレットとインスペクターの横幅を縮小し、キャンバス優先の 3 カラム配分へ再調整
- 主要パネル、ボタン、入力、ノードの角丸を 3px 基準へ寄せた
